...
kubernetes1.11兼容最好的etcd版本是3.2.18（官方github上有说明）
```bash
...
$ scp target/configs/download-images.sh <node-ip>:~
...
# 把ca分发到每个worker节点
$ scp target/pki/ca.pem <node-ip>:/etc/kubernetes/pki
```
#### 8.3 kubelet配置文件
...
## 9. 部署kube-proxy（所有节点）
kublet 运行在每个 worker 节点上，接收 kube-apiserver 发送的请求，管理 Pod 容器的整个生命周期。
#### 9.1 创建证书和私钥
```bash
$ cd target/pki/proxy
$ cfssl gencert -ca=../ca.pem \
...
$ scp target/configs/calico* <user>@<node-ip>:/etc/kubernetes/addons/
...
```bash
# 上传配置文件
$ scp target/configs/coredns.yaml <user>@<node-ip>:/etc/kubernetes/addons
